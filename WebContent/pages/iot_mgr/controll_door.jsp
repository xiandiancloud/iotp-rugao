		...
		<div style="display:inline-block;  float: left;text-align: center;border-right: solid 1px #cee0f4;padding: 5px;width: 135px">
			<dl title="${item.devStatusDesc}">
				<%-- <c:if test="${item.devStatus!='0'}">
					<img id="${item.devId}_img" src="${ctx}/images/ico_door_open.png" />
				</c:if>
				<c:if test="${item.devStatus=='0'}">
					<img id="${item.devId}_img" src="${ctx}/images/ico_door.png" />
				</c:if> --%>
				<img id="${item.devId}_img" src="${ctx}/images/door_menu.png" />
			</dl>
	...
	$.getJSON("${ctx}/roomMonitor/switchDev/"+devId+"/"+switchFlag+"?r="+Math.random(), function(result){
		if(result=='1')
		{
			if(devId.indexOf(",")>0)
			{
				...
				{
					if(switchFlag=='0')
						$("#"+devIds[i]+'_img').attr("src","${ctx}/images/ico_door.png");
					else
						$("#"+devIds[i]+'_img').attr("src","${ctx}/images/ico_door_open.png");
				}
			}
			else
			{
				if(switchFlag=='0')
					$("#"+devId+'_img').attr("src","${ctx}/images/ico_door.png");
				else
					$("#"+devId+'_img').attr("src","${ctx}/images/ico_door_open.png");
